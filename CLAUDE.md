...
- **每次功能改动后必须打包测试**，确保 PyInstaller 产物可正常启动：
  ```bash
  uv run pyinstaller --name night-watcher --noconsole --hidden-import requests --icon resources/icon.ico --add-data "adapter;adapter" --add-data "themes;themes" --add-data "resources;resources" --add-data "i18n;i18n" main.py -y
  ```
